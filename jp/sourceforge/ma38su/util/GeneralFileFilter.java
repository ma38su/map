...
public class GeneralFileFilter extends FileFilter implements java.io.FileFilter {
	...
	/**
	 * JFileChooserにラスタ画像のファイルフィルターを追加します。
	 * Javaで書き出せるファイルのみ許可しまするため、GIF形式は含みません
	 * 標準の拡張子はPNG形式として設定します。
	 * @param chooser ファイルフィルタを追加するJFileChooser
	 */
	...
	/**
	 * ラスタ画像を読み込むためのファイルフィルタを取得します。
	 * Javaで読み込める画像をすべて設定します。
	...
	/**
	 * 許可する拡張子を与えてインスタンスを生成します。
	 * 許可する拡張子の最初の要素が標準の拡張子に設定されます。
	 * （JPEG,HTMLなど複数の拡張子が存在する場合）
	 * @param extensions 許可する拡張子
	 * @param description 拡張子の説明
